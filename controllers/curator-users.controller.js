...
const getCuratoruserById = async (req, res, next) => {
  try {
    const { id } = req.params;
    const user = await fetchCuratoruserById(id);
    if (!user) {
...
const patchCuratoruserAddress = async (req, res, next) => {
  try {
    ...
    const updatedUser = await updateCuratoruserAddress(id, address);
    if (!updatedUser) {
      return res.status(404).json({ msg: "User not found" });
    }
...
const patchCuratoruserNickname = async (req, res, next) => {
  try {
    ...
    const updatedUser = await updateCuratoruserNickname(id, nickname);
    if (!updatedUser) {
      return res.status(404).json({ msg: "User not found" });
    }
    res.status(200).json(updatedUser);
